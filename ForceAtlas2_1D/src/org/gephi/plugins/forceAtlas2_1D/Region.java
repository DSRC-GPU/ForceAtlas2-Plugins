...
public class Region {
    ...
    private void updateMassAndGeometry() {
        if (nodes.size() > 1) {
            ...
            for (Node n : nodes) {
                ...
                mass += nLayout.mass;
                massSumX += n.getNodeData().x() * nLayout.mass;
                massSumY += n.getNodeData().y() * nLayout.mass;
            }
            ...
            for (Node n : nodes) {
                double distance = Math.sqrt((n.getNodeData().x() - massCenterX) * (n.getNodeData().x() - massCenterX) + (n.getNodeData().y() - massCenterY) * (n.getNodeData().y() - massCenterY));
    ...
    public synchronized void buildSubRegions() {
        if (nodes.size() > 1) {
            ...
            for (Node n : nodes) {
                ArrayList<Node> nodesColumn = (n.getNodeData().x() < massCenterX) ? (leftNodes) : (rightNodes);
                nodesColumn.add(n);
            }
            ...
            for (Node n : leftNodes) {
                ArrayList<Node> nodesLine = (n.getNodeData().y() < massCenterY) ? (topleftNodes) : (bottomleftNodes);
                nodesLine.add(n);
            }
            ...
            for (Node n : rightNodes) {
                ArrayList<Node> nodesLine = (n.getNodeData().y() < massCenterY) ? (toprightNodes) : (bottomrightNodes);
                nodesLine.add(n);
            }
